...
def plotResults(y_test_encoded, y_pred, label_encoder, languages, output_dir, chunk_size=20):
    ...
    for i in range(num_chunks):
        ...
        # Confusion Matrix
        fig_cm = plt.figure(figsize=(15, 12), dpi=300)
        ...
        plt.title(f'Confusion Matrix (Chunk {i+1})', 
                 pad=20, fontsize=16, fontweight='bold')
        plt.xlabel('Predicted', labelpad=10, fontsize=12)
        plt.ylabel('True', labelpad=10, fontsize=12)
        ...
        # Plot Metrics
        fig_metrics = plt.figure(figsize=(15, 8), dpi=300)
        ...
        plt.title(f'Performance Metrics (Chunk {i+1})', 
                 pad=20, fontsize=16, fontweight='bold')
        plt.xticks(rotation=45, ha='right', fontsize=10)
        plt.yticks(rotation=0, fontsize=10)
...
def plotClassDistribution(y_train, y_test, label_encoder, output_dir):
    ...
    distribution_data = pd.DataFrame({
        'Train Samples': [train_class_counts.get(c, 0) for c in all_classes],
